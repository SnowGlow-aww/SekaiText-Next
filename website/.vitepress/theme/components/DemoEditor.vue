...
const TERMS: Term[] = [
  ...
  { t: '編集', to: '编辑', note: '制片组剪辑视频统一用语', category: '用语' },
  { t: '動画', to: '视频', note: '制片组的作品分类', category: '用语' },
]
...
const groups = ref<Group[]>([
  {
    speaker: 'カケル',
    avatar: 'カ',
    jp: 'なあ、この前の星鳴祭の動画さ、海外からコメント来てたぞ。',
    voice: true,
    items: [{ id: 1, text: '诶，上次星鸣祭那个视频，有海外的人留言了。', touched: false }],
  },
  {
    speaker: 'トモリ',
    avatar: 'ト',
  ...
  },
  {
    speaker: 'ラナ',
    avatar: 'ラ',
    jp: '何せ私たちが三日間かけて編集した動画ですからね、皆さんお疲れ様でした。',
    voice: true,
...
// 说话人头像配色（站点品牌系）
const AVATAR_COLORS: Record<string, string> = { 'カ': '#39c5bb', 'ト': '#ff69b4', 'ラ': '#6fa8ef', 'ス': '#f5a623' }

// 首个空行呼吸引导：不提示的话看起来像静态截图，没人知道能点
const hintDismissed = ref(false)
const hintGi = computed(() =>
  hintDismissed.value ? -1 : groups.value.findIndex((g) => !g.baseParts && g.items.every((it) => !it.text.trim())),
)
      ...
      <div class="d-main">
        ...
        <div ref="workspaceRef" class="d-workspace">
          <div class="d-cols">
            <div class="d-col-h">
              <span>原文</span>
              <span class="d-scenario">demo_story_01</span>
            </div>
          ...
          <div class="d-groups">
            <div v-for="(g, gi) in groups" :key="gi" class="d-group">
              <!-- 原文侧 -->
              <div class="d-src">
                <span class="d-avatar" :style="{ background: AVATAR_COLORS[g.avatar] }">{{ g.avatar }}</span>
              ...
              <div class="d-dst">
                ...
                <div
                  ...
                  :key="it.id"
                  class="d-row"
                  :class="{ 'd-row-changed': showCompare && isChanged(g, it), 'd-row-hint': gi === hintGi && ii === 0 }"
                >
        ...
        <div class="d-foot">
          <template v-if="allDone">
            <a :href="withBase('/download.html')">下载桌面版</a>，打轴、压制、Live2D 应有尽有，插件生态建设中……
          </template>
          <template v-else>
            动手试试吧！输入译文、悬停<span class="d-foot-term">虚线术语</span>、差异高亮、点 ＋ 给译文 \N 分行。（示例文本为宣传用创作）
          </template>
        </div>
      </div>
    </div>
...
/* 覆盖 VitePress 全局 button:focus 的系统焦点环：鼠标点击不留蓝圈，键盘导航仍可见 */
.stx-demo button:focus {
  outline: none;
}
.stx-demo button:focus-visible {
  outline: 2px solid var(--dp);
  outline-offset: 1px;
}

/* 首个空行呼吸引导 */
.d-row-hint {
  border-color: color-mix(in srgb, var(--dp) 45%, var(--d-border));
  animation: d-hint 2.4s ease-in-out infinite;
}
@keyframes d-hint {
  0%, 100% { box-shadow: 0 0 0 0 transparent; }
  50% { box-shadow: 0 0 0 4px color-mix(in srgb, var(--dp) 13%, transparent); border-color: var(--dp); }
}
