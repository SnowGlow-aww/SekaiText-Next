...
const WHATS_NEW: Record<string, { title?: string; body: string; link?: TourStep['link'] }> = {
  ...
  '5.7': {
    body:
      ...
      '· 并行压制修复：进度显示不再串线（每个任务独立进度条）；硬编启动失败自动降级 x264 软编重试不再白挂；压制日志报错自动导出、可随时查看 / 导出；推荐编码器独显优先；需配合轴机插件 3.2.0\n' +
      '· 成品字幕修复：1行/2行/3行 样式改按剧情原文行数判定（此前数译文换行，译文没断行或三行长台词分句切半会被误判成 1行、译文压在日文行上）——升级后重新导出即可\n' +
      '· 保存串档修复：文档命名与写入目标在载入剧情时固定，之后拉取 / 下载别的剧情不再影响当前文档；载入新剧情不会再把译文写进上一个剧情的文件（此前前后篇连续翻译会互相覆盖）；卡面剧情文件名去掉 -01/-02，直接以前篇 / 后篇区分\n' +
      '· 文件名跟随文档：标题译文 / 模式变了，已保存的文件就地自动改名（此前文件名冻结在首次自动建档时，后填的标题译文永远进不了文件名）；文稿目录统一为「编号 标题」完整命名，旧目录里的文件保存时自动归位；打开老 txt 按文件名【前缀】识别模式，校对稿不再被当成翻译稿改写\n' +
      '· 稳定性修复（内部审计）：Aegisub 同步拉取期间的再次保存不再被漏掉；连续跨行快速编辑不再丢上一行译文；更换文稿位置遇同名文件不再改绑覆盖；崩溃恢复按文档自身坐标重建（不受下载页选择影响）；配合轴机插件 3.2.1——并行模式切换任务时行列表 / 同步状态不再串到别的任务\n' +
