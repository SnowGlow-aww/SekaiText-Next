# SekaiText Next v6.0.1 更新日志

> **地图对话拉取与剧情下载同步修复**：修复剧情下载中「追加地图对话」按时间排序时索引仅显示部分活动的问题，全量同步官方活动剧情列表并支持加载对应活动的全部追加章节。

## 🗺️ 追加地图对话按时间排序同步修复
- **活动索引全量同步**：在剧情下载页面中选择「追加地图对话」并按时间排序时，索引下拉列表现已直接对齐全部官方活动剧情（按最新活动倒序排列），数量与活动剧情保持一致；
- **全量章节获取支持**：修复按时间筛选时的章节过滤逻辑，支持准确加载选中活动所追加的全部对话剧情（包含普通追加对话与限定追加对话），并生成对应的 CDN 下载路径与原文导出；
- **多端与双向解析一致性**：保持按人物、按地点筛选时的互斥与正交性，确保文稿逆向标识解析（`ResolveLabelDetailed`）及相关单元测试行为完全稳定。

---

# SekaiText Next v6.0.0 更新日志

> **架构升级与体验提升**：H.265/HEVC 硬件压制调优、并发任务队列、输入焦点防回退、模式隔离撤回、SkSelect 搜索与 Live2D 本地写穿秒开缓存。

## 🚀 重点更新
- **H.265/HEVC 压制与任务队列**：优化硬件编码器调度与任务队列并发管控；
- **输入与撤回体验优化**：打轴 staff 输入失焦防回退、表格键盘导航、行内全局快捷键与模式隔离撤回栈；
- **选择器与角色变体**：SkSelect 搜索过滤支持拼音/日文首字母检索，SpeakerCheck 角色名变体匹配；
- **Live2D 离线缓存**：本地写穿落盘机制，素材秒开；
- **Android APK 更新与多端分发**：支持 Android 原生安装器应用内升级。

---

# SekaiText Next v5.9.14 更新日志

> **功能模块调整**：应版权所有人要求，删除术语库模块。

## 📚 功能调整
- **下线术语库与语法用例模块**：应版权所有人要求，移除应用内的术语库与语法用例模块，精简应用导航与体积。
