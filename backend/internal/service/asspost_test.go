...
`
	...
	// 确认拆分为前后两段拼凑轴（Part 1 对应 1行，Part 2 对应 1行+2行原生分行）
	if !strings.Contains(out, "好让各个SEKAI的大家") {
		t.Fatalf("缺少前半段台词:\n%s", out)
	}
	if !strings.Contains(out, "能自由来往交流") || !strings.Contains(out, "加深彼此的羁绊") {
		t.Fatalf("缺少后半段台词:\n%s", out)
	}
...
func TestPostProcess_LukaDialogueSplitAndDoubleOutline(t *testing.T) {
	const lukaAss = `[Script Info]
PlayResX: 2560
PlayResY: 1600

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
Comment: 0,0:00:01.00,0:00:05.00,Screen,,0,0,0,,----- 001 ----- Start
Dialogue: 0,0:00:01.00,0:00:05.00,Character,,0,0,0,,{\pos(335,1285)}ルカ
Dialogue: 0,0:00:01.00,0:00:05.00,Line2,,0,0,0,,『想和至今为止遇见过的所有SEKAI里的我们，\N全都成为好朋友！』
Comment: 0,0:00:01.00,0:00:05.00,Screen,,0,0,0,,----- 001 ----- End
`
	post, err := PostProcessAss(lukaAss, AssPostOptions{
		Clean: true, SyncTags: true, SpeakerColor: true,
	})
	if err != nil {
		t.Fatalf("PostProcessAss: %v", err)
	}
