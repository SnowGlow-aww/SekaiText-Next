...
// SourceTalksAsDst 把解析出的原文行直接当作译文行（「导出原文 txt」用）。
// 对话框正文按编辑器的多行结构拆成多个 DstTalk：首行 Start=true，中间行
// End=false，末行 End=true。这样 SerializeContent(..., true) 会写出字面量
// \N，导出的文件可以无损重新导入编辑器。
...
// ManagedTranslationFileName returns the canonical filename used by the editor
// for a translation document. The standalone original-text export intentionally
// uses the same "翻译" mode label so the result can be imported as a normal
// translation file without a rename step.
func ManagedTranslationFileName(saveTitle, chapterTitle string) string {
	canonical := strings.TrimSpace(saveTitle)
	if chapter := strings.TrimSpace(chapterTitle); chapter != "" {
		if canonical != "" {
			canonical += " "
		}
		canonical += chapter
	...
	// SaveTitle/ChapterTitle come from the bundled catalog, but keep the helper
	// safe if a future catalog or caller supplies path separators/control chars.
