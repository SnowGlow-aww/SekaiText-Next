...
-- 兼容 Aegisub 3.2.2 ~ 最新版：个别环境 require "lfs" 不可用，此时回落读取
-- 轴机每次推送都会同步覆盖的固定名副本 _sekaitext.sekaisync.txt。
local function find_sync_file()
    ...
    if ok and lfs then
        ...
        local fallback = dir .. path_sep() .. "_sekaitext.sekaisync.txt"
        local fh = io.open(fallback, "r")
        if fh then
            fh:close()
            return fallback
        end
